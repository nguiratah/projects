function[] = nofeedbackgenerationplot4(lambda, TD, D, N,  G, n_samples, K)
G =1;
n_samples =25;
N  = 25;
%L2 = 29;
...
for i = 1 : n_samples
  ...
  c(r, i) = 0;
  for l =1 :H
  a(r, i) =a(r, i) + nofeedbackgenerations(lambda, TD, D, N, 0.25, G, s(i), K(r))/H;
  b(r, i) =b(r, i)+ nofeedbackgenerations(lambda, TD, D, N, 0.5, G, s(i), K(r))/H;
  c(r, i) =c(r, i)+ nofeedbackgenerations(lambda, TD, D, N, 0.75, G, s(i), K(r))/H;
...
title(['maximum of number of not expired packets for lambda = 25) = f(coding window size) for TD =' int2str(TD)]);
xlabel('coding window size');
ylabel('maximum of number of not expired packets');
%% second loop :
% s =  10;
