classdef ODESOLVER_GUIApp < handle
%
% Usage:
%     app.ODESOLVER_GUIApp

  properties
      AppPath = {'/ODESOLVER_GUI'};
	  AppClass = 'ODESOLVER_GUIApp';
	  AppHandle;
	  AppCount = 0;
      Increment = 1;
	  Decrement = 0;
  ...
  methods
    ...
    function startApp(obj)
        ...
        if nargout(@odesolver) == 0  
            eval('odesolver');
        else
			obj.AppHandle = eval('odesolver');   
        end 
        % Increment the reference count by one    
        ODESOLVER_GUIApp.refcount(obj.Increment);
