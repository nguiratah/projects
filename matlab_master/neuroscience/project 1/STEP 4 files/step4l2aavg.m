...
sigma =400;
for j=1:20
    fileID = fopen(['matlabSTRF/data/l2a_avg/conspecific/stim' num2str(j)]);
    c1 =textscan(fileID, '%s');
    stimavg{j}  = audioread(c1{1, 1}{1, 1}); 
    responseavg{j} = totalspikes(['matlabSTRF/data/l2a_avg/conspecific/spike' num2str(j)], 2000);
end
for j=1:10
    fileID = fopen(['matlabSTRF/data/l2a_avg/flatrip/stim' num2str(j)]);
    c1 =textscan(fileID, '%s');
    stimavg{j+20}  = audioread(c1{1, 1}{1, 1}); 
    responseavg{j+20} = totalspikes(['matlabSTRF/data/l2a_avg/flatrip/spike' num2str(j)], 2000);

end
for j=1:20
    fileID = fopen(['matlabSTRF/data/l2a_avg/songrip/stim' num2str(j)]);
    c1 =textscan(fileID, '%s');
    stimavg{j+30}  = audioread(c1{1, 1}{1, 1}); 
    responseavg{j+30} = totalspikes(['matlabSTRF/data/l2a_avg/songrip/spike' num2str(j)], 2000);
end
stimuli_train = [];
response_train = [];

for j = 1 : 40
    response_train = [response_train responseavg{j}];
    N = length(responseavg{j});
    X = abs(fft(stimavg{j}));
...
response_test = [];
for j = 41 : 50
    response_test = [response_test responseavg{j}];
    N = length(responseavg{j});
    X = abs(fft(stimavg{j}));
