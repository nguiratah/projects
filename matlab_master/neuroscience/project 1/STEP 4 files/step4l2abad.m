...
sigma =400;
for j=1:20
    fileID = fopen(['matlabSTRF/data/l2a_bad/conspecific/stim' num2str(j)]);
    c1 =textscan(fileID, '%s');
    stimbad{j}  = audioread(c1{1, 1}{1, 1}); 
    responsebad{j} = totalspikes(['matlabSTRF/data/l2a_bad/conspecific/spike' num2str(j)], 2000);
end
for j=1:10
    fileID = fopen(['matlabSTRF/data/l2a_bad/flatrip/stim' num2str(j)]);
    c1 =textscan(fileID, '%s');
    stimbad{j+20}  = audioread(c1{1, 1}{1, 1}); 
    responsebad{j+20} = totalspikes(['matlabSTRF/data/l2a_bad/flatrip/spike' num2str(j)], 2000);
