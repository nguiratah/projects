...
for j=1:20
    ...
    responsegood{j} = totalspikes(['matlabSTRF/data/l2a_good/conspecific/spike' num2str(j)], 2000);
end
for j=1:10
    fileID = fopen(['matlabSTRF/data/l2a_good/flatrip/stim' num2str(j)]);
    c1 =textscan(fileID, '%s');
    stimgood{j+20}  = audioread(c1{1, 1}{1, 1}); 
    responsegood{j+20} = totalspikes(['matlabSTRF/data/l2a_good/flatrip/spike' num2str(j)], 2000);
...
for j=1:20
    ...
    responsegood{j+30} = totalspikes(['matlabSTRF/data/l2a_good/songrip/spike' num2str(j)], 2000);
end
stimuli_train = [];
response_train = [];
