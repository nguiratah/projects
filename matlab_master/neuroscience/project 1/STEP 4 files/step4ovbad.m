...
sigma =400;
for j=1:20
    fileID = fopen(['matlabSTRF/data/ov_bad/conspecific/stim' num2str(j)]);
    c1 =textscan(fileID, '%s');
    stimbad{j}  = audioread(c1{1, 1}{1, 1}); 
    responsebad{j} = totalspikes(['matlabSTRF/data/ov_bad/conspecific/spike' num2str(j)], 2000);
end
for j=1:10
    fileID = fopen(['matlabSTRF/data/ov_bad/flatrip/stim' num2str(j)]);
    c1 =textscan(fileID, '%s');
    stimbad{j+20}  = audioread(c1{1, 1}{1, 1}); 
    responsebad{j+20} = totalspikes(['matlabSTRF/data/ov_bad/flatrip/spike' num2str(j)], 2000);

end
% for j=1:20
%     fileID = fopen(['matlabSTRF/data/ov_bad/songrip/stim' num2str(j)]);
%     c1 =textscan(fileID, '%s');
%     stimbad{j+30}  = audioread(c1{1, 1}{1, 1}); 
%     responsebad{j+30} = totalspikes(['matlabSTRF/data/ov_bad/songrip/spike' num2str(j)], 2000);
% end
stimuli_train = [];
response_train = [];
