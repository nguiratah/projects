...
sigma =400;
for j=1:20
    fileID = fopen(['matlabSTRF/data/ov_good/conspecific/stim' num2str(j)]);
    c1 =textscan(fileID, '%s');
    stimgood{j}  = audioread(c1{1, 1}{1, 1}); 
    responsegood{j} = totalspikes(['matlabSTRF/data/ov_good/conspecific/spike' num2str(j)], 2000);
end
for j=1:10
    fileID = fopen(['matlabSTRF/data/ov_good/flatrip/stim' num2str(j)]);
    c1 =textscan(fileID, '%s');
    stimgood{j+20}  = audioread(c1{1, 1}{1, 1}); 
    responsegood{j+20} = totalspikes(['matlabSTRF/data/ov_good/flatrip/spike' num2str(j)], 2000);
