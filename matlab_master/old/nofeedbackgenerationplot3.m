function[] = nofeedbackgenerationplot3(lambda, TD, D, N,  G, n_samples, K)
G =1000;
n_samples =25;
N  = 25;
%L2 = 29;
...
for i = 1 : n_samples
  ...
  for r = 1 : g*s(i)
  K(r) =r;
  a(r, i) = nofeedbackgenerations(lambda, TD, D, N, 0.25, G, s(i), K(r));%/(H*floor(K(r)*N/s(i)));
  b(r, i) = nofeedbackgenerations(lambda, TD, D, N, 0.5, G, s(i), K(r));%/(H*floor(K(r)*N/s(i)));
...
title(['(K*/coding window size) = f(coding window size) for TD infinite']);
