function[] = nofeedbackslidingplot1(lambda, TD, D, N,  G, n_samples, K, s)
G =1000;
n_samples =25;
N  = 25;
%L2 = 29;
...
for i = 1 : n_samples
  ...
  for r = 1 : g*s(i)
  K(r) =r;
%  [~, q]=  nofeedbacksliding(lambda, TD, D, N, 0.25, G, s(i), K(r), m);
  a(r, i) = nofeedbacksliding(lambda, TD, D, N, 0.25, G, s(i), K(r), m);
%  [~, q]=  nofeedbacksliding(lambda, TD, D, N, 0.5, G, s(i), K(r), m);
  b(r, i) = nofeedbacksliding(lambda, TD, D, N, 0.5, G, s(i), K(r), m);
%  [~, q]=  nofeedbacksliding(lambda, TD, D, N, 0.75, G, s(i), K(r), m);
...
title(['(k*/coding window size, m=1) = f(coding window size) for TD infinite']);
